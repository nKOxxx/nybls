...
**Round 1, coverage.**
`nybls sheet <id>` (add `--range START_S END_S` for long videos: one sheet per ~5 to 7 min region of interest). Read each sheet. 1 sheet = 6 timestamped thumbnails = 1 image unit.
...
- Sheets for *where to look*; full frames for *looking*; zooms for *reading*. Never request >10 frames in one call; usually 2 to 4 targeted frames beat 10 speculative ones.
